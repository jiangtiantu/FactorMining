...
def _apply_modification(population, operator, pb, record):
    """
    Apply the modification given by *operator* to each individual in *population* with probability *pb* in place.
    """
    for i in range(len(population)):
        pb = adaptive_mut(record['max_train'], record['avg_train'], population[i])
        if random.random() < pb:
...
def _apply_crossover(population, operator, pb, record):
    """
    Mate the *population* in place using *operator* with probability *pb*.
    """
    for i in range(1, len(population), 2):
        ind1, ind2 = population[i - 1], population[i]
        pb = adaptive_cross(record['max_train'], record['avg_train'], ind1, ind2)
        if random.random() < pb:
            population[i - 1], population[i] = operator(ind1, ind2)
            population[i - 1].flag, population[i].flag = 1, 1

    return population


def adaptive_cross(fitMax: float, fitAver: float, ind1, ind2) -> float:
    fit = max(ind1.fitness.values[0], ind2.fitness.values[0])
    p = 1 if fit < fitAver or fitMax == fitAver else abs((fitMax - fit) / (fitMax - fitAver)) * 0.85
    return p


def adaptive_mut(fitMax: float, fitAver: float, ind) -> float:
    fit = ind.fitness.values[0]
    p = 0.5 if fit < fitAver or fitMax == fitAver else abs((fitMax - fit) / (fitMax - fitAver)) * 0.5
    return p
...
def gep_simple(population, toolbox, n_generations=100, n_elites=1,
    ...
    for gen in range(n_generations + 1):
        ...
        for op in toolbox.pbs:
            if op.startswith('cx'):
                offspring = _apply_crossover(offspring, getattr(toolbox, op), toolbox.pbs[op], record)

        # mutation
        for op in toolbox.pbs:
            if op.startswith('mut'):
                offspring = _apply_modification(offspring, getattr(toolbox, op), toolbox.pbs[op], record)
