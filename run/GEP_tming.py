...
class GEPTiming(GEP):
    ...
    def evolution(self,
        ...
        # yRetAll = self.ret_synthesis(self.data.Y, ySignAll)

        # 收益率标签
        yRetTrain1 = self.data.Y[np.argwhere((self.data.Label == 0) & (ySignAll == 1)).flatten()]
        yRetTrain0 = self.data.Y[np.argwhere((self.data.Label == 0) & (ySignAll == 0)).flatten()]

        yRetVerify1 = self.data.Y[np.argwhere((self.data.Label == 1) & (ySignAll == 1)).flatten()]
        yRetVerify0 = self.data.Y[np.argwhere((self.data.Label == 1) & (ySignAll == 0)).flatten()]

        yRetVerify21 = self.data.Y[np.argwhere((self.data.Label == 2) & (ySignAll == 1)).flatten()]
        yRetVerify20 = self.data.Y[np.argwhere((self.data.Label == 2) & (ySignAll == 0)).flatten()]

        yRetTest1 = self.data.Y[np.argwhere((self.data.Label == 3) & (ySignAll == 1)).flatten()]
        yRetTest0 = self.data.Y[np.argwhere((self.data.Label == 3) & (ySignAll == 0)).flatten()]

        # 01标签
        flag0Train = ySignAll[np.argwhere((self.data.Label == 0) & (self.data.sign == 0)).flatten()]
        flag1Train = ySignAll[np.argwhere((self.data.Label == 0) & (self.data.sign == 1)).flatten()]

        flag0Verify = ySignAll[np.argwhere((self.data.Label == 1) & (self.data.sign == 0)).flatten()]
        flag1Verify = ySignAll[np.argwhere((self.data.Label == 1) & (self.data.sign == 1)).flatten()]

        flag0Verify2 = ySignAll[np.argwhere((self.data.Label == 2) & (self.data.sign == 0)).flatten()]
        flag1Verify2 = ySignAll[np.argwhere((self.data.Label == 2) & (self.data.sign == 1)).flatten()]

        flag0Test = ySignAll[np.argwhere((self.data.Label == 3) & (self.data.sign == 0)).flatten()]
        flag1Test = ySignAll[np.argwhere((self.data.Label == 3) & (self.data.sign == 1)).flatten()]
        ...
        # 适应度计算
        fitnessTrain = self.tTest(yRetTrain1, yRetTrain0, 0.05 * self.data.trainL)
        ...
        # 验证集
        individual.verify.values = self.tTest(yRetVerify1, yRetVerify0, 0.05 * self.data.verifyL),
        individual.verify2.values = self.tTest(yRetVerify21, yRetVerify20, 0.05 * self.data.verify2L),
    ...
    def benchmark(self):
        trainBM = self.data.Y[np.argwhere(self.data.Label == 0).flatten()]
        verifyBM = self.data.Y[np.argwhere(self.data.Label == 1).flatten()]
        testBM = self.data.Y[np.argwhere(self.data.Label == 2).flatten()]

        trainSR = ((trainBM + 1).prod() ** (240 / self.data.trainL) - 1 - self.rf_a) / np.std(trainBM) / np.sqrt(240)
        verifySR = ((verifyBM + 1).prod() ** (240 / self.data.verifyL) - 1 - self.rf_a) / np.std(verifyBM) / np.sqrt(
            240)
        testSR = ((testBM + 1).prod() ** (240 / self.data.testL) - 1 - self.rf_a) / np.std(testBM) / np.sqrt(240)

        self.res['BM'] = pd.Series([trainSR, verifySR, testSR], index=['trainSR', 'verifySR', 'testSR'])
        #
        # trainBM1 = self.data.Y[np.argwhere((self.data.Label == 0) & (self.data.sign != 0)).flatten()]
        # trainBMAll = self.data.Y[np.argwhere(self.data.Label == 0).flatten()]
        #
        # verifyBM1 = self.data.Y[np.argwhere((self.data.Label == 1) & (self.data.sign != 0)).flatten()]
        # verifyBMAll = self.data.Y[np.argwhere(self.data.Label == 1).flatten()]
        #
        # testBM1 = self.data.Y[np.argwhere((self.data.Label == 2) & (self.data.sign != 0)).flatten()]
        # testBMAll = self.data.Y[np.argwhere(self.data.Label == 2).flatten()]
        #
        # testRBM1 = self.data.Y[np.argwhere((self.data.Label == 3) & (self.data.sign != 0)).flatten()]
        # testRBMAll = self.data.Y[np.argwhere(self.data.Label == 3).flatten()]
        #
